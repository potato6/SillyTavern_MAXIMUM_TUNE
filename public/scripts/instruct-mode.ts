...
export function getInstructStoppingSequences({
  ...
  if (customInstruct || instruct.enabled) {
    ...
    const first_output_sequence = instruct.first_output_sequence?.replace(NAME_REGEX, name2) || '';
    const last_output_sequence = instruct.last_output_sequence?.replace(NAME_REGEX, name2) || '';
    const system_sequence = instruct.system_sequence?.replace(NAME_REGEX, 'System') || '';
    const last_system_sequence = instruct.last_system_sequence?.replace(NAME_REGEX, 'System') || '';
...
export function formatInstructModeStoryString(
  ...
  if (applySequences) {
    if (instructSettings.story_string_prefix) {
      const prefix = substituteParams(instructSettings.story_string_prefix).replace(NAME_REGEX, 'System');
...
export function formatInstructModeExamples(mesExamplesArray, name1, name2) {
  ...
  if (power_user.instruct.macro) {
    inputPrefix = substituteParams(inputPrefix, { name1Override: name1, name2Override: name2 }).replace(NAME_REGEX, name1);
    outputPrefix = substituteParams(outputPrefix, { name1Override: name1, name2Override: name2 }).replace(NAME_REGEX, name2);
    inputSuffix = substituteParams(inputSuffix, { name1Override: name1, name2Override: name2 }).replace(NAME_REGEX, name1);
    outputSuffix = substituteParams(outputSuffix, { name1Override: name1, name2Override: name2 }).replace(NAME_REGEX, name2);
  ...
  for (let i = 0; i < mesExamplesArray.length; i++) {
    ...
    const cleanedItem = item.replace(START_REGEX, '{Example Dialogue:}').replace(CARRIAGE_RETURN_REGEX, '');
    ...
    for (let j = 0; j < blockExamples.length; j++) {
      ...
      const includeThisName =
        !includeGroupNames &&
        (includeNames || (power_user.instruct.names_behavior === names_behavior_types.FORCE && isUser));

      const prefix = isUser ? inputPrefix : outputPrefix;
      const suffix = isUser ? inputSuffix : outputSuffix;
      const name = isUser ? name1 : name2;

      const messageContent = includeThisName ? `${name}: ${example.content}${suffix}` : `${example.content}${suffix}`;
...
export function formatInstructModePrompt(
  ...
  if (instruct.macro) {
    sequence = substituteParams(sequence, { name1Override: name1, name2Override: name2 }).replace(NAME_REGEX, name || 'System');
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('instruct_system_same_as_user')?.addEventListener('input', function() {
    ...
    if (seq instanceof HTMLInputElement || seq instanceof HTMLTextAreaElement) seq.readOnly = state;
    if (suf instanceof HTMLInputElement || suf instanceof HTMLTextAreaElement) suf.readOnly = state;
  });
