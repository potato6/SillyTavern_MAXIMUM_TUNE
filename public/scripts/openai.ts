...
const textCompletionModels = new Set([
...
const interleaved_reasoning_providers = new Set([
 ...
 */
export function formatWorldInfo(
    value: string,
    { wiFormat = null }: { wiFormat?: string | null } = {},
) {
    if (!value) {
        return '';
    }
...
async function populationInjectionPrompts(prompts, messages) {
    ...
    for (let i = 0; i <= maxDepth; i++) {
        ...
        const depthPrompts = prompts.filter(
            (prompt: any) => prompt.injection_depth === i && prompt.content,
        );
        ...
        const orders = Object.keys(orderGroups).toSorted((a, b) => +b - +a);
        for (const order of orders) {
            const orderPrompts = (orderGroups as any)[order];

            // Order of priority for roles (most important go lower)
            const roles = ['system', 'user', 'assistant'];
            for (const role of roles) {
                const rolePrompts = orderPrompts
                    .filter((prompt: any) => prompt.role === role)
                    .map((x: any) => x.content)
                    .join(separator);
...
async function populateChatHistory(
    ...
    if (type === 'continue' && cyclePrompt && !oai_settings.continue_prefill) {
        const promptObject = {
            ...
            role: 'system',
            content: substituteParams(oai_settings.continue_nudge_prompt, {
    ...
    const isToolReasoningProvider = interleaved_reasoning_providers.has(
    ...
    const chatPool = [...messages].toReversed();
    for (let index = 0; index < chatPool.length; index++) {
        ...
        if (
            (promptManager!.serviceSettings as Record<string, unknown>).names_behavior ===
        ...
        if (canUseTools && Array.isArray(chatPrompt.invocations)) {
            ...
            // @ts-expect-error TS(7006) FIXME: Parameter 'invocation' implicitly has an 'any' typ... Remove this comment to see the full error message
...
async function preparePromptsForChatCompletion({
    ...
    if (summary && summary.value)
        systemPrompts.push({
            ...
            identifier: 'summary',
            // @ts-expect-error TS(2345) FIXME: Argument of type '{ role: string; content: any; id... Remove this comment to see the full error message
    ...
    const knownExtensionPrompts = new Set([
        '1_memory',
        '2_floating_prompt',
        '3_vectors',
        '4_vectors_data_bank',
    ...
    for (const key in extensionPrompts) {
        if (Object.hasOwn(extensionPrompts, key)) {
            ...
            if (knownExtensionPrompts.has(key)) continue;
            ...
            systemPrompts.push({
                ...
                // @ts-expect-error TS(2345) FIXME: Argument of type '{ identifier: string; position: ... Remove this comment to see the full error message
    ...
    systemPrompts.forEach((prompt) => {
        ...
        if (collectionPrompt) {
            // In-Chat / Relative
            (prompt as unknown as Record<string, unknown>).injection_position =
                (collectionPrompt as unknown as Record<string, unknown>).injection_position ??
                (prompt as unknown as Record<string, unknown>).injection_position;
            // Depth for In-Chat
            (prompt as unknown as Record<string, unknown>).injection_depth =
                (collectionPrompt as unknown as Record<string, unknown>).injection_depth ??
                (prompt as unknown as Record<string, unknown>).injection_depth;
            // Priority for In-Chat
            (prompt as unknown as Record<string, unknown>).injection_order =
                (collectionPrompt as unknown as Record<string, unknown>).injection_order ??
                (prompt as unknown as Record<string, unknown>).injection_order;
            // Role (system, user, assistant)
            prompt.role =
                ((collectionPrompt as unknown as Record<string, unknown>).role as string) ??
                prompt.role;
        }
...
export async function prepareOpenAIMessages(
    {
        // @ts-expect-error TS(7031) FIXME: Binding element 'name2' implicitly has an 'any' ty... Remove this comment to see the full error message
        name2,
        // @ts-expect-error TS(7031) FIXME: Binding element 'charDescription' implicitly has a... Remove this comment to see the full error message
        charDescription,
        // @ts-expect-error TS(7031) FIXME: Binding element 'charPersonality' implicitly has a... Remove this comment to see the full error message
        charPersonality,
        // @ts-expect-error TS(7031) FIXME: Binding element 'scenario' implicitly has an 'any'... Remove this comment to see the full error message
        scenario,
        // @ts-expect-error TS(7031) FIXME: Binding element 'worldInfoBefore' implicitly has a... Remove this comment to see the full error message
        worldInfoBefore,
        // @ts-expect-error TS(7031) FIXME: Binding element 'worldInfoAfter' implicitly has an... Remove this comment to see the full error message
        worldInfoAfter,
        // @ts-expect-error TS(7031) FIXME: Binding element 'bias' implicitly has an 'any' typ... Remove this comment to see the full error message
        bias,
        ...
        type,
        // @ts-expect-error TS(7031) FIXME: Binding element 'quietPrompt' implicitly has an 'a... Remove this comment to see the full error message
        quietPrompt,
        // @ts-expect-error TS(7031) FIXME: Binding element 'quietImage' implicitly has an 'an... Remove this comment to see the full error message
        quietImage,
        // @ts-expect-error TS(7031) FIXME: Binding element 'extensionPrompts' implicitly has ... Remove this comment to see the full error message
        extensionPrompts,
        // @ts-expect-error TS(7031) FIXME: Binding element 'cyclePrompt' implicitly has an 'a... Remove this comment to see the full error message
        cyclePrompt,
        // @ts-expect-error TS(7031) FIXME: Binding element 'systemPromptOverride' implicitly ... Remove this comment to see the full error message
        systemPromptOverride,
        // @ts-expect-error TS(7031) FIXME: Binding element 'jailbreakPromptOverride' implicit... Remove this comment to see the full error message
        jailbreakPromptOverride,
        ...
        messages,
        // @ts-expect-error TS(7031) FIXME: Binding element 'messageExamples' implicitly has a... Remove this comment to see the full error message
        messageExamples,
    },
    dryRun: any,
) {
    ...
    chatCompletion.setTokenBudget(
        userSettings.openai_max_context as number,
        userSettings.openai_max_tokens as number,
    );
    ...
    openai_messages_count =
        chat.filter((x) => !x?.tool_calls && ['user', 'assistant', 'tool'].includes(x?.role))
            .length || 0;
    ...
    try {
        ...
        if (data.detail) {
            if (!quiet)
                notyf.error(
                    ((
                        (data.detail as Record<string, unknown>)?.error as
...
function calculateOpenRouterCost() {
    ...
    document.getElementById('openrouter_max_prompt_cost').textContent = cost;
}
...
function saveModelList(data) {
    ...
    if (oai_settings.chat_completion_source == chat_completion_sources.OPENROUTER) {
        ...
        const modelOpenRouterSelect = document.getElementById('model_openrouter_select');
        if (modelOpenRouterSelect) modelOpenRouterSelect.innerHTML = '';
        const websiteOption = document.createElement('option');
        websiteOption.value = openrouter_website_model;
        websiteOption.textContent = t`Use OpenRouter website setting`;
        modelOpenRouterSelect?.appendChild(websiteOption);

        if (oai_settings.group_models) {
            groupModelsByVendor(model_list, chat_completion_sources.OPENROUTER).forEach(
                (models: any, vendor: any) => {
                    ...
                    document.getElementById('model_openrouter_select')?.appendChild(optgroup);
                },
            );
        } else {
        ...
        (() => {
            const el = document.getElementById('model_openrouter_select');
            if (el) {
                (el as any).value = oai_settings.openrouter_model;
    ...
    if (oai_settings.chat_completion_source == chat_completion_sources.OPENAI) {
        const externalCategory = document.getElementById('openai_external_category');
        ...
        if (oai_settings.show_external_models) {
            ...
            (() => {
                const el = document.getElementById('model_openai_select');
                if (el) {
                    (el as any).value = model;
                    el.dispatchEvent(new Event('change'));
                }
            })();
        }
    ...
    if (oai_settings.chat_completion_source == chat_completion_sources.CUSTOM) {
        ...
        if (!oai_settings.custom_model && model_list.length > 0) {
            (() => {
                const el = document.getElementById('model_custom_select');
                if (el) {
                    (el as any).value = model_list[0].id;
                    el.dispatchEvent(new Event('change'));
                }
            })();
        }
    ...
    if (oai_settings.chat_completion_source == chat_completion_sources.AIMLAPI) {
        ...
        if (aimlapiSelect) aimlapiSelect.innerHTML = '';
        ...
        if (oai_settings.group_models) {
            groupModelsByVendor(model_list, chat_completion_sources.AIMLAPI).forEach(
                (models: any, vendor: any) => {
                    ...
                    aimlapiSelect?.appendChild(optgroup);
                },
            );
        } else {
            model_list.forEach((model) => {
        ...
        (() => {
            const el = document.getElementById('model_aimlapi_select');
            if (el) {
                (el as any).value = oai_settings.aimlapi_model;
                el.dispatchEvent(new Event('change'));
            }
        })();
    }
    ...
    if (oai_settings.chat_completion_source == chat_completion_sources.MISTRALAI) {
        ...
        (() => {
            const el = document.getElementById('model_mistralai_select');
            if (el) {
                (el as any).value = oai_settings.mistralai_model;
                el.dispatchEvent(new Event('change'));
            }
        })();
    }
    ...
    if (oai_settings.chat_completion_source == chat_completion_sources.ELECTRONHUB) {
        ...
        if (electronHubSelect) electronHubSelect.innerHTML = '';
        ...
        if (oai_settings.group_models) {
            groupModelsByVendor(model_list, chat_completion_sources.ELECTRONHUB).forEach(
                (models: any, vendor: any) => {
                    ...
                    electronHubSelect?.appendChild(optgroup);
                },
            );
        } else {
            model_list.forEach((model) => {
        ...
        (() => {
            const el = document.getElementById('model_electronhub_select');
            if (el) {
                (el as any).value = oai_settings.electronhub_model;
                el.dispatchEvent(new Event('change'));
            }
        })();
    }
    ...
    if (oai_settings.chat_completion_source == chat_completion_sources.CHUTES) {
        ...
        if (chutesSelect) chutesSelect.innerHTML = '';
        ...
        if (oai_settings.group_models) {
            groupModelsByVendor(model_list, chat_completion_sources.CHUTES).forEach(
                (models: any, vendor: any) => {
                    ...
                    chutesSelect?.appendChild(optgroup);
                },
            );
        } else {
            model_list.forEach((model) => {
        ...
        (() => {
            const el = document.getElementById('model_chutes_select');
            if (el) {
                (el as any).value = oai_settings.chutes_model;
                el.dispatchEvent(new Event('change'));
            }
        })();
    }
    ...
    if (oai_settings.chat_completion_source == chat_completion_sources.NANOGPT) {
        ...
        const nanogptSelect = document.getElementById('model_nanogpt_select');
        if (nanogptSelect) nanogptSelect.innerHTML = '';
        ...
        if (oai_settings.group_models) {
            groupModelsByVendor(model_list, chat_completion_sources.NANOGPT).forEach(
                (models: any, vendor: any) => {
                    ...
                    nanogptSelect?.appendChild(optgroup);
                },
            );
        } else {
            model_list.forEach((model) => {
        ...
        if (nanogptSelect) {
            ...
            nanogptSelect.value = oai_settings.nanogpt_model;
            nanogptSelect.dispatchEvent(new Event('change'));
        }
    }
    ...
    if (oai_settings.chat_completion_source == chat_completion_sources.DEEPSEEK) {
        ...
        if (deepseekSelect) {
            // @ts-expect-error TS(2339) FIXME: Property 'value' does not exist on type 'HTMLEleme... Remove this comment to see the full error message
            deepseekSelect.value = oai_settings.deepseek_model;
            deepseekSelect.dispatchEvent(new Event('change'));
        }
    }
    ...
    if (oai_settings.chat_completion_source === chat_completion_sources.POLLINATIONS) {
        ...
        if (pollinationsSelect) {
            // @ts-expect-error TS(2339) FIXME: Property 'value' does not exist on type 'HTMLEleme... Remove this comment to see the full error message
            pollinationsSelect.value = oai_settings.pollinations_model;
            pollinationsSelect.dispatchEvent(new Event('change'));
        }
    }

    if (oai_settings.chat_completion_source === chat_completion_sources.MAKERSUITE) {
        const googleOtherModels = document.getElementById('google_other_models');
        if (googleOtherModels) googleOtherModels.innerHTML = '';

        const googleSelect = document.getElementById('model_google_select');
        // @ts-expect-error TS(7034) FIXME: Variable 'staticModels' implicitly has type 'any[]... Remove this comment to see the full error message
        const staticModels = [];
        if (googleSelect) {
            ...
            for (const el of googleSelect.options) {
        ...
        if (googleSelect) {
            // @ts-expect-error TS(2339) FIXME: Property 'value' does not exist on type 'HTMLEleme... Remove this comment to see the full error message
            googleSelect.value = oai_settings.google_model;
    ...
    if (oai_settings.chat_completion_source === chat_completion_sources.GROQ) {
        ...
        if (groqSelect) {
            // @ts-expect-error TS(2339) FIXME: Property 'value' does not exist on type 'HTMLEleme... Remove this comment to see the full error message
            groqSelect.value = oai_settings.groq_model;
            groqSelect.dispatchEvent(new Event('change'));
        }
    }
    ...
    if (oai_settings.chat_completion_source === chat_completion_sources.SILICONFLOW) {
        ...
        if (siliconflowSelect) {
            // @ts-expect-error TS(2339) FIXME: Property 'value' does not exist on type 'HTMLEleme... Remove this comment to see the full error message
            siliconflowSelect.value = oai_settings.siliconflow_model;
            siliconflowSelect.dispatchEvent(new Event('change'));
        }
    }
    ...
    if (oai_settings.chat_completion_source === chat_completion_sources.FIREWORKS) {
        ...
        if (fireworksSelect) {
            // @ts-expect-error TS(2339) FIXME: Property 'value' does not exist on type 'HTMLEleme... Remove this comment to see the full error message
            fireworksSelect.value = oai_settings.fireworks_model;
            fireworksSelect.dispatchEvent(new Event('change'));
        }
    }
    ...
    if (oai_settings.chat_completion_source === chat_completion_sources.WORKERS_AI) {
        ...
        if (workersAiSelect) {
            // @ts-expect-error TS(2339) FIXME: Property 'value' does not exist on type 'HTMLEleme... Remove this comment to see the full error message
            workersAiSelect.value = oai_settings.workers_ai_model;
            workersAiSelect.dispatchEvent(new Event('change'));
        }
    }

    if (oai_settings.chat_completion_source === chat_completion_sources.COMETAPI) {
        const cometapiSelect = document.getElementById('model_cometapi_select');
        ...
        if (cometapiSelect) {
            // @ts-expect-error TS(2339) FIXME: Property 'value' does not exist on type 'HTMLEleme... Remove this comment to see the full error message
            cometapiSelect.value = oai_settings.cometapi_model;
            cometapiSelect.dispatchEvent(new Event('change'));
        }
    }
    ...
    if (oai_settings.chat_completion_source == chat_completion_sources.AZURE_OPENAI) {
        ...
        const azureSelect = document.getElementById('azure_openai_model');
        if (azureSelect) {
            azureSelect.innerHTML = '';
            azureSelect.appendChild(new Option(modelId || 'None', modelId || '', true, true));
            azureSelect.dispatchEvent(new Event('change'));
        }
    }
    ...
    if (oai_settings.chat_completion_source == chat_completion_sources.XAI) {
        ...
        if (xaiSelect) {
            // @ts-expect-error TS(2339) FIXME: Property 'value' does not exist on type 'HTMLEleme... Remove this comment to see the full error message
            xaiSelect.value = oai_settings.xai_model;
            xaiSelect.dispatchEvent(new Event('change'));
        }
    }
    ...
    if (oai_settings.chat_completion_source == chat_completion_sources.MOONSHOT) {
        ...
        if (moonshotSelect) {
            // @ts-expect-error TS(2339) FIXME: Property 'value' does not exist on type 'HTMLEleme... Remove this comment to see the full error message
            moonshotSelect.value = oai_settings.moonshot_model;
            moonshotSelect.dispatchEvent(new Event('change'));
        }
    }
}
...
function getVerbosity(settings = null) {
    // @ts-expect-error TS(2322) FIXME: Type '{ send_if_empty: string; impersonation_promp... Remove this comment to see the full error message
    settings = settings ?? oai_settings;
...
export async function createGenerationParameters(
    ...
    const isO1 =
        gptSources.has(settings.chat_completion_source) && ['o1-2024-12-17', 'o1'].includes(model);
    ...
    const generate_data = {
        ...
        // @ts-expect-error TS(2345) FIXME: Argument of type '4' is not assignable to paramete... Remove this comment to see the full error message
    ...
    if (settings.chat_completion_source === chat_completion_sources.AZURE_OPENAI) {
        // @ts-expect-error TS(2339) FIXME: Property 'azure_base_url' does not exist on type '... Remove this comment to see the full error message
        generate_data.azure_base_url = settings.azure_base_url;
        // @ts-expect-error TS(2339) FIXME: Property 'azure_deployment_name' does not exist on... Remove this comment to see the full error message
        generate_data.azure_deployment_name = settings.azure_deployment_name;
        // @ts-expect-error TS(2339) FIXME: Property 'azure_api_version' does not exist on typ... Remove this comment to see the full error message
    ...
    if (settings.reverse_proxy && proxySupportedSources.includes(settings.chat_completion_source)) {
        ...
        // @ts-expect-error TS(2339) FIXME: Property 'reverse_proxy' does not exist on type '{... Remove this comment to see the full error message
        generate_data.reverse_proxy = settings.reverse_proxy;
        // @ts-expect-error TS(2339) FIXME: Property 'proxy_password' does not exist on type '... Remove this comment to see the full error message
    ...
    if (useLogprobs && logprobsSupportedSources.includes(settings.chat_completion_source)) {
        // @ts-expect-error TS(2339) FIXME: Property 'logprobs' does not exist on type '{ type... Remove this comment to see the full error message
    ...
    if (gptSources.has(settings.chat_completion_source) && isVision(model)) {
        // @ts-expect-error TS(2790) FIXME: The operand of a 'delete' operator must be optiona... Remove this comment to see the full error message
    ...
    if (gptSources.has(settings.chat_completion_source) && /gpt-4.5/.test(model)) {
        // @ts-expect-error TS(2339) FIXME: Property 'logprobs' does not exist on type '{ type... Remove this comment to see the full error message
        delete generate_data.logprobs;
    }

    if (settings.chat_completion_source === chat_completion_sources.CLAUDE) {
        // @ts-expect-error TS(2551) FIXME: Property 'top_k' does not exist on type '{ type: a... Remove this comment to see the full error message
        generate_data.top_k = Number(settings.top_k_openai);
        // @ts-expect-error TS(2339) FIXME: Property 'use_sysprompt' does not exist on type '{... Remove this comment to see the full error message
        ...
        if (type !== 'quiet' && !(type === 'continue' && settings.continue_prefill)) {
            // @ts-expect-error TS(2339) FIXME: Property 'assistant_prefill' does not exist on typ... Remove this comment to see the full error message
    ...
    if (settings.chat_completion_source === chat_completion_sources.OPENROUTER) {
        // @ts-expect-error TS(2551) FIXME: Property 'top_k' does not exist on type '{ type: a... Remove this comment to see the full error message
        generate_data.top_k = Number(settings.top_k_openai);
        // @ts-expect-error TS(2339) FIXME: Property 'min_p' does not exist on type '{ type: a... Remove this comment to see the full error message
        generate_data.min_p = Number(settings.min_p_openai);
        // @ts-expect-error TS(2339) FIXME: Property 'repetition_penalty' does not exist on ty... Remove this comment to see the full error message
        generate_data.repetition_penalty = Number(settings.repetition_penalty_openai);
        // @ts-expect-error TS(2551) FIXME: Property 'top_a' does not exist on type '{ type: a... Remove this comment to see the full error message
        generate_data.top_a = Number(settings.top_a_openai);
        // @ts-expect-error TS(2339) FIXME: Property 'use_fallback' does not exist on type '{ ... Remove this comment to see the full error message
        generate_data.use_fallback = settings.openrouter_use_fallback;
        // @ts-expect-error TS(2339) FIXME: Property 'provider' does not exist on type '{ type... Remove this comment to see the full error message
        ...
        // @ts-expect-error TS(2339) FIXME: Property 'middleout' does not exist on type '{ typ... Remove this comment to see the full error message
        generate_data.middleout = settings.openrouter_middleout;
    }

    if (settings.chat_completion_source === chat_completion_sources.NANOGPT) {
        // @ts-expect-error TS(2339) FIXME: Property 'nanogpt_provider' does not exist on type... Remove this comment to see the full error message
        generate_data.nanogpt_provider = settings.nanogpt_provider;
        // @ts-expect-error TS(2339) FIXME: Property 'nanogpt_payg_override' does not exist on... Remove this comment to see the full error message
    ...
    if (
        ...
        if (settings.chat_completion_source === chat_completion_sources.VERTEXAI) {
            // @ts-expect-error TS(2339) FIXME: Property 'vertexai_auth_mode' does not exist on ty... Remove this comment to see the full error message
            generate_data.vertexai_auth_mode = settings.vertexai_auth_mode;
            // @ts-expect-error TS(2339) FIXME: Property 'vertexai_region' does not exist on type ... Remove this comment to see the full error message
            generate_data.vertexai_region = settings.vertexai_region;
            // @ts-expect-error TS(2339) FIXME: Property 'vertexai_express_project_id' does not ex... Remove this comment to see the full error message
    ...
    if (settings.chat_completion_source === chat_completion_sources.MISTRALAI) {
        // @ts-expect-error TS(2339) FIXME: Property 'safe_prompt' does not exist on type '{ t... Remove this comment to see the full error message
    ...
    if (settings.chat_completion_source === chat_completion_sources.CUSTOM) {
        // @ts-expect-error TS(2339) FIXME: Property 'custom_url' does not exist on type '{ ty... Remove this comment to see the full error message
        generate_data.custom_url = settings.custom_url;
        // @ts-expect-error TS(2339) FIXME: Property 'custom_include_body' does not exist on t... Remove this comment to see the full error message
        generate_data.custom_include_body = settings.custom_include_body;
        // @ts-expect-error TS(2339) FIXME: Property 'custom_exclude_body' does not exist on t... Remove this comment to see the full error message
        generate_data.custom_exclude_body = settings.custom_exclude_body;
        // @ts-expect-error TS(2339) FIXME: Property 'custom_include_headers' does not exist o... Remove this comment to see the full error message
    ...
    if (settings.chat_completion_source === chat_completion_sources.ZAI) {
        ...
        // @ts-expect-error TS(2345) FIXME: Argument of type '1' is not assignable to paramete... Remove this comment to see the full error message
        generate_data.stop = getCustomStoppingStrings(1);
        // @ts-expect-error TS(2339) FIXME: Property 'zai_endpoint' does not exist on type '{ ... Remove this comment to see the full error message
    ...
    if (settings.chat_completion_source === chat_completion_sources.SILICONFLOW) {
        // @ts-expect-error TS(2339) FIXME: Property 'siliconflow_endpoint' does not exist on ... Remove this comment to see the full error message
    ...
    if (settings.chat_completion_source === chat_completion_sources.MINIMAX) {
        // @ts-expect-error TS(2339) FIXME: Property 'minimax_endpoint' does not exist on type... Remove this comment to see the full error message
    ...
    if (settings.chat_completion_source === chat_completion_sources.WORKERS_AI) {
        // @ts-expect-error TS(2339) FIXME: Property 'workers_ai_account_id' does not exist on... Remove this comment to see the full error message
        ...
        // @ts-expect-error TS(2339) FIXME: Property 'repetition_penalty' does not exist on ty... Remove this comment to see the full error message
        generate_data.repetition_penalty = Number(settings.repetition_penalty_openai);
        // @ts-expect-error TS(2339) FIXME: Property 'seed' does not exist on type '{ type: an... Remove this comment to see the full error message
    ...
    if (
        ...
        // @ts-expect-error TS(2339) FIXME: Property 'max_completion_tokens' does not exist on... Remove this comment to see the full error message
        ...
        if (/^(openai\/)?(o1)/.test(model)) {
            ...
            delete generate_data.n;
            // @ts-expect-error TS(2339) FIXME: Property 'tools' does not exist on type '{ type: a... Remove this comment to see the full error message
            delete generate_data.tools;
            // @ts-expect-error TS(2339) FIXME: Property 'tool_choice' does not exist on type '{ t... Remove this comment to see the full error message
            delete generate_data.tool_choice;
        }
    }

    if (gptSources.has(settings.chat_completion_source) && /gpt-5/.test(model)) {
        // @ts-expect-error TS(2339) FIXME: Property 'max_completion_tokens' does not exist on... Remove this comment to see the full error message
        generate_data.max_completion_tokens = generate_data.max_tokens;
        delete generate_data.max_tokens;
        // @ts-expect-error TS(2339) FIXME: Property 'logprobs' does not exist on type '{ type... Remove this comment to see the full error message
    ...
    if (jsonSchema) {
        // @ts-expect-error TS(2339) FIXME: Property 'json_schema' does not exist on type '{ t... Remove this comment to see the full error message
        ...
        return async function* streamData() {
            ...
            while (true) {
                ...
                if (
                    ...
                    swipes[swipeIndex] =
                        (swipes[swipeIndex] || '') +
                        getStreamingReply(parsed, state, { overrideShowThoughts: null });
                } else {
...
export function getStreamingReply(
    ...
    if (chat_completion_source === chat_completion_sources.CLAUDE) {
        ...
        if (show_thoughts) {
            state.reasoning +=
                data.choices?.filter((x: any) => x?.delta?.reasoning_content)?.[0]?.delta
                    ?.reasoning_content || '';
        }
        ...
        if (show_thoughts) {
            state.reasoning +=
                ...
                data.choices?.filter((x: any) => x?.message?.reasoning_content)?.[0]?.message
                    ?.reasoning_content ??
                '';
        }
        ...
        if (show_thoughts) {
            state.reasoning +=
                data.choices?.filter((x: any) => x?.delta?.content?.[0]?.thinking)?.[0]?.delta
...
function parseChatCompletionLogprobs(data) {
    ...
    switch (oai_settings.chat_completion_source) {
        ...
        case chat_completion_sources.CHUTES:
            ...
            return textCompletionModels.has(getChatCompletionModel())
                ? parseOpenAITextLogprobs(data.choices[0]?.logprobs)
                : parseOpenAIChatLogprobs(data.choices[0]?.logprobs);
        default:
...
function parseOpenAIChatLogprobs(logprobs) {
    ...
    return content.map(({ token, logprob, top_logprobs = [] }) => {
        ...
        // @ts-expect-error TS(7006) FIXME: Parameter 'top' implicitly has an 'any' type.
...
class TokenHandler {
     ...
     */
    // @ts-expect-error TS(7006) FIXME: Parameter 'countTokenAsyncFn' implicitly has an 'a... Remove this comment to see the full error message
...
class Message {
    ...
    // @ts-expect-error TS(2564) FIXME: Property 'tool_calls' has no initializer and is no... Remove this comment to see the full error message
     ...
     * @returns {Promise<void>}
     */
    // @ts-expect-error TS(7006) FIXME: Parameter 'invocations' implicitly has an 'any' ty... Remove this comment to see the full error message
    async setToolCalls(invocations, includeSignature, includeReasoning = false) {
        ...
        const fallbackReasoning =
            invocations.find((i: any) => typeof i.reasoning === 'string' && i.reasoning.length > 0)
                ?.reasoning || null;
     ...
     * @returns {Promise<void>}
     */
    // @ts-expect-error TS(7006) FIXME: Parameter 'video' implicitly has an 'any' type.
    ...
    async compressImage(image) {
        ...
        if (
            ...
            const maxSide = 2048;
            // @ts-expect-error TS(2345) FIXME: Argument of type '2048' is not assignable to param... Remove this comment to see the full error message
...
export class ChatCompletion {
    ...
    async squashSystemMessages() {
        const excludeList = new Set(['newMainChat', 'newChat', 'groupNudge']);
        ...
        for (const message of this.messages.collection) {
            ...
            const shouldSquash = (message) => {
                return (
                    !excludeList.has(message.identifier) &&
                    message.role === 'system' &&
                    !message.name
                );
            };
     ...
     */
    // @ts-expect-error TS(7006) FIXME: Parameter 'tokens' implicitly has an 'any' type.
...
function migrateChatCompletionSettings(settings) {
    const migrateMap = [
        ...
        },
        {
            ...
            oldValue: (custom_prompt_post_processing_types as any).CLAUDE,
...
function loadOpenAISettings(data, settings) {
    ...
    openai_setting_names.forEach(function (item, i) {
        ...
        option.text = item;
        const settingsPresetOpenai = document.getElementById('settings_preset_openai');
        settingsPresetOpenai?.appendChild(option);
    });
    ...
    for (const key of Object.keys(default_settings)) {
        (oai_settings as Record<string, unknown>)[key] =
        ...
        if (settingToUpdate) {
            ...
            if ($element.matches('input[type="checkbox"]')) {
                ($element as HTMLInputElement).checked = (oai_settings as Record<string, unknown>)[
                    key
                ] as boolean;
            } else if ($element.matches('select')) {
                ($element as HTMLSelectElement).value = String(
                    (oai_settings as Record<string, unknown>)[key] ?? '',
                );
                $element
                    .querySelector(
                        `option[value="${CSS.escape(String((oai_settings as Record<string, unknown>)[key] ?? ''))}"]`,
                    )
                    ?.setAttribute('selected', 'true');
            } else {
                ($element as HTMLInputElement).value = String(
                    (oai_settings as Record<string, unknown>)[key] ?? '',
                );
                if ($element.matches('input[type="range"]')) {
                    const id = $element.getAttribute('id');
                    const $counter = document.querySelector(
                        `input[type="number"][data-for="${id}"]`,
                    );
                    if ($counter) {
                        ($counter as HTMLInputElement).value = Number(
                            (oai_settings as Record<string, unknown>)[key] ?? 0,
                        ).toString();
                    }
                }
            }
    ...
    if (bindPresetEl)
        (bindPresetEl as HTMLInputElement).checked = oai_settings.bind_preset_to_connection;
    (() => {
        const el = document.getElementById('openai_external_category');
        if (el) {
            el.style.display = oai_settings.show_external_models ? '' : 'none';
        }
    })();
    const _rpEl1 = document.querySelector('.reverse_proxy_warning') as HTMLElement;
    if (_rpEl1) _rpEl1.style.display = oai_settings.reverse_proxy !== '' ? '' : 'none';
    ...
    if (logitBiasPreset) logitBiasPreset.innerHTML = '';
    for (const preset of Object.keys(oai_settings.bias_presets)) {
        ...
        logitBiasPreset?.appendChild(option);
    }
    if (logitBiasPreset) logitBiasPreset.dispatchEvent(new Event('change'));
...
function setNamesBehaviorControls() {
    ...
    // @ts-expect-error TS(2531) FIXME: Object is possibly 'null'.
    document.getElementById('character_names_display').textContent = checkedItemText;
}
...
function setContinuePostfixControls() {
    ...
    const checkedItemSpan = document.querySelector('input[name="continue_postfix"]:checked ~ span');
    const checkedItemText = checkedItemSpan?.textContent?.trim() ?? '';
    // @ts-expect-error TS(2531) FIXME: Object is possibly 'null'.
    document.getElementById('continue_postfix_display').textContent = checkedItemText;
}
...
function setToolReasoningControls() {
    ...
    (() => {
        const el = document.getElementById('openrouter_interleaved_thinking_disabled_hint');
        if (el) {
...
async function getStatusOpen() {
    ...
    if (oai_settings.chat_completion_source === chat_completion_sources.CUSTOM) {
        document.querySelectorAll('.model_custom_select').forEach((el) => (el.innerHTML = ''));
        // @ts-expect-error TS(2339) FIXME: Property 'custom_url' does not exist on type '{ re... Remove this comment to see the full error message
...
async function saveOpenAIPreset(name, settings, triggerUi = true) {
    ...
    if (savePresetSettings.ok) {
        ...
        if (Object.keys(openai_setting_names).includes(data.name)) {
            ...
            (document as any)
                .querySelector('#settings_preset_openai option[value="${value}"]')
                ?.selected(true);
            if (triggerUi)
                document
                    .getElementById('settings_preset_openai')
                    ?.dispatchEvent(new Event('change'));
        } else {
            openai_settings.push(presetBody);
            (openai_setting_names as Record<string, number>)[data.name] =
                openai_settings.length - 1;
            const option = document.createElement('option');
            option.selected = true;
            option.value = String(openai_settings.length - 1);
            option.innerText = data.name;
            const settingsPresetOpenai = document.getElementById('settings_preset_openai');
            if (triggerUi && settingsPresetOpenai) {
                settingsPresetOpenai.appendChild(option);
                settingsPresetOpenai.dispatchEvent(new Event('change'));
            }
        }
    } else {
...
function onLogitBiasPresetChange() {
    ...
    const list = document.querySelector('.openai_logit_bias_list');
...
function createLogitBiasListItem(entry) {
    ...
    const template = document
        .querySelector('#openai_logit_bias_template .openai_logit_bias_form')!
        .cloneNode(true);
    ...
    const textInput = template.querySelector('.openai_logit_bias_text') as HTMLInputElement;
    textInput.value = entry.text;
    textInput.oninput = function () {
        entry.text = String(textInput.value);
        biasCache = undefined;
        saveSettingsDebounced();
    };
    // @ts-expect-error TS(2339) FIXME: Property 'querySelector' does not exist on type 'N... Remove this comment to see the full error message
    const valueInput = template.querySelector('.openai_logit_bias_value') as HTMLInputElement;
    ...
    // @ts-expect-error TS(2339) FIXME: Property 'querySelector' does not exist on type 'N... Remove this comment to see the full error message
    template.querySelector('.openai_logit_bias_remove')?.addEventListener('click', function () {
        (template as HTMLElement).closest('.openai_logit_bias_form')?.remove();
        // @ts-expect-error TS(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
    ...
    document.querySelector('.openai_logit_bias_list')?.prepend(template);
}
...
async function onPresetImportFileChange(e) {
    ...
    if (Object.keys(openai_setting_names).includes(data.name)) {
        ...
        (openai_setting_names as Record<string, number>)[data.name] =
            (openai_settings as Record<string, unknown>[]).length - 1;
        const option = document.createElement('option');
        option.selected = true;
...
async function onExportPresetClick() {
    ...
    const presetIndex = (openai_setting_names as Record<string, number>)[
        oai_settings.preset_settings_openai
    ] as number;
    const preset = structuredClone(
        (openai_settings as Record<string, unknown>[])[presetIndex],
    ...
    const removeConnectionData =
        (
            exportConnectionTemplate.querySelector(
                'input[name="export_connection_data"]:checked',
...
function onLogitBiasPresetExportClick() {
    ...
    const presetJsonString = JSON.stringify(
        (oai_settings as any).bias_presets[(oai_settings as any).bias_preset_selected],
        null,
        4,
    );
...
async function onDeletePresetClick() {
    ...
    const value = (openai_setting_names as Record<string, number>)[
        oai_settings.preset_settings_openai
    ];
...
function onSettingsPresetChange() {
    ...
    const preset = structuredClone(
        openai_settings[
            (openai_setting_names as Record<string, number>)[
                oai_settings.preset_settings_openai
            ] as number
        ]!,
    );
    ...
    eventSource
        ...
        .finally(async () => {
            if (oai_settings.bind_preset_to_connection) {
                document
                    .querySelectorAll('.model_custom_select')
            ...
            if (oai_settings.bind_preset_to_connection) {
                document
                    ...
                    ?.dispatchEvent(new Event('change'));
                document
                    .getElementById('openrouter_providers_chat')
                    ?.dispatchEvent(new Event('change'));
                document
                    .getElementById('openrouter_quantizations_chat')
                    ?.dispatchEvent(new Event('change'));
                document.getElementById('nanogpt_provider')?.dispatchEvent(new Event('change'));
            }
...
async function onModelChange() {
    ...
    // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
    if (this.matches('#model_claude_select')) {
        if (value.includes('-v')) {
        ...
        // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
        document.getElementById('model_claude_select').value = oai_settings.claude_model;
    }

    // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
    if (this.matches('#model_openai_select')) {
        console.log('OpenAI model changed to', value);
        oai_settings.openai_model = value;
    }

    // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
    if (this.matches('#model_openrouter_select')) {
    ...
    // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
    if (this.matches('#model_ai21_select')) {
        if (value === '' || value.startsWith('j2-')) {
            value = 'jamba-large';
            // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
            document.getElementById('model_ai21_select').value = value;
        }
    ...
    // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
    if (this.matches('#model_google_select')) {
        if (!value) {
    ...
    if (this.matches('#model_mistralai_select')) {
        ...
        // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
        document.getElementById('model_mistralai_select').value = oai_settings.mistralai_model;
    }

    // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
    if (this.matches('#model_cohere_select')) {
        console.log('Cohere model changed to', value);
        oai_settings.cohere_model = value;
    }

    // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
    if (this.matches('#model_perplexity_select')) {
        console.log('Perplexity model changed to', value);
        oai_settings.perplexity_model = value;
    }

    // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
    if (this.matches('#model_groq_select')) {
        if (!value || !hasModelsLoaded) {
    ...
    // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
    if (this.matches('#model_siliconflow_select')) {
        if (!value) {
    ...
    // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
    if (this.matches('#model_minimax_select')) {
        if (!value) {
    ...
    // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
    if (this.matches('#model_electronhub_select')) {
        if (!value || !hasModelsLoaded) {
    ...
    // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
    if (this.matches('#model_chutes_select')) {
        if (!value || !hasModelsLoaded) {
    ...
    // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
    if (this.matches('#model_nanogpt_select')) {
        if (!value || !hasModelsLoaded) {
    ...
    // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
    if (this.matches('#model_deepseek_select')) {
        if (!value) {
    ...
    // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
    if (value && this.matches('#model_custom_select')) {
        console.log('Custom model changed to', value);
        oai_settings.custom_model = value;
        (() => {
            const el = document.getElementById('custom_model_id') as HTMLInputElement | null;
            if (el) {
                el.value = value;
    ...
    // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
    if (this.matches('#model_xai_select')) {
        if (!value) {
    ...
    // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
    if (this.matches('#model_moonshot_select')) {
        if (!value || !hasModelsLoaded) {
    ...
    // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
    if (this.matches('#model_fireworks_select')) {
        if (!value || !hasModelsLoaded) {
    ...
    // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
    if (this.matches('#model_cometapi_select')) {
        if (!value) {
    ...
    // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
    if (this.matches('#azure_openai_model')) {
        if (!value) {
    ...
    // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
    if (this.matches('#model_zai_select')) {
        console.log('ZAI model changed to', value);
        oai_settings.zai_model = value;
    }

    // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
    if (this.matches('#model_workers_ai_select')) {
        if (!value || !hasModelsLoaded) {
    ...
    if (
        ...
        // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
        document.getElementById('openai_max_context').setAttribute('max', String(contextSize));
        oai_settings.temp_openai = Math.min(maxTemp, oai_settings.temp_openai);
        (() => {
            const el = document.getElementById('temp_openai');
            if (el) {
                el.setAttribute('max', String(maxTemp));
                (el as any).value = oai_settings.temp_openai;
                el.dispatchEvent(new Event('input'));
            }
        })();
        ...
        (() => {
            const el = document.getElementById('openai_max_context');
            if (el) {
                (el as any).value = oai_settings.openai_max_context;
                el.dispatchEvent(new Event('input'));
            }
        })();
    }
    ...
    if (oai_settings.chat_completion_source == chat_completion_sources.OPENROUTER) {
        if (oai_settings.max_context_unlocked) {
            // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
            document.getElementById('openai_max_context').setAttribute('max', String(unlocked_max));
        } else {
            ...
            if (model?.context_length) {
                ...
                document
                    ...
                    .setAttribute('max', String(model.context_length));
            } else {
                // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
                document.getElementById('openai_max_context').setAttribute('max', String(max_128k));
            }
        }
        oai_settings.openai_max_context = Math.min(
            Number(document.getElementById('openai_max_context')?.getAttribute('max') ?? 0),
        ...
        if (value && (value.includes('claude') || value.includes('palm-2'))) {
            ...
            (() => {
                const el = document.getElementById('temp_openai');
                if (el) {
                    el.setAttribute('max', String(claude_max_temp));
                    (el as any).value = oai_settings.temp_openai;
                    el.dispatchEvent(new Event('input'));
                }
            })();
            ...
            (() => {
                const el = document.getElementById('temp_openai');
                if (el) {
                    el.setAttribute('max', String(oai_max_temp));
                    (el as any).value = oai_settings.temp_openai;
                    el.dispatchEvent(new Event('input'));
                }
            })();
    ...
    if (oai_settings.chat_completion_source == chat_completion_sources.CLAUDE) {
        if (oai_settings.max_context_unlocked) {
            ...
            // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
            document.getElementById('openai_max_context').setAttribute('max', String(max_1mil));
        } else if (/^claude-(3|opus|haiku|sonnet)/.test(value)) {
            // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
            document.getElementById('openai_max_context').setAttribute('max', String(max_200k));
        } else {
            // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
            document.getElementById('openai_max_context').setAttribute('max', String(max_200k));
        }
        ...
        // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
        document
            .getElementById('openai_reverse_proxy')
            .setAttribute('placeholder', 'https://api.anthropic.com/v1');
    ...
    if (
        ...
        document
            .getElementById('openai_max_context')
            .setAttribute('max', String(getMaxContextOpenAI(value) ?? ''));
        oai_settings.openai_max_context = Math.min(
            oai_settings.openai_max_context,
            Number(document.getElementById('openai_max_context')?.getAttribute('max') ?? 0),
        );
        ...
        document
            .getElementById('openai_reverse_proxy')
            .setAttribute('placeholder', 'https://api.openai.com/v1');
    ...
    if (oai_settings.chat_completion_source === chat_completion_sources.MISTRALAI) {
        ...
        // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
        document.getElementById('openai_max_context').setAttribute('max', String(maxContext));
        oai_settings.openai_max_context = Math.min(
            oai_settings.openai_max_context,
            Number(document.getElementById('openai_max_context')?.getAttribute('max') ?? 0),
        );
        ...
        (() => {
            const el = document.getElementById('temp_openai');
            if (el) {
                el.setAttribute('max', String(mistral_max_temp));
                (el as any).value = oai_settings.temp_openai;
                el.dispatchEvent(new Event('input'));
            }
        })();
    ...
    if (oai_settings.chat_completion_source === chat_completion_sources.COHERE) {
        if (oai_settings.max_context_unlocked) {
            ...
            // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
            document.getElementById('openai_max_context').setAttribute('max', String(max_4k));
        } else if (
            ...
            // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
            document.getElementById('openai_max_context').setAttribute('max', String(max_256k));
        } else if (['c4ai-aya-23-8b', 'c4ai-aya-expanse-8b'].includes(oai_settings.cohere_model)) {
            // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
            document.getElementById('openai_max_context').setAttribute('max', String(max_8k));
        } else if (
            ['c4ai-aya-vision-8b', 'c4ai-aya-vision-32b'].includes(oai_settings.cohere_model)
        ) {
            // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
            document.getElementById('openai_max_context').setAttribute('max', String(max_16k));
        } else {
            // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
            document.getElementById('openai_max_context').setAttribute('max', String(max_4k));
        }
    ...
    if (oai_settings.chat_completion_source === chat_completion_sources.PERPLEXITY) {
        if (oai_settings.max_context_unlocked) {
            ...
            // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
            document.getElementById('openai_max_context').setAttribute('max', String(127000));
        } else if (['sonar-pro'].includes(oai_settings.perplexity_model)) {
            // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
            document.getElementById('openai_max_context').setAttribute('max', String(200000));
    ...
    if (oai_settings.chat_completion_source === chat_completion_sources.WORKERS_AI) {
        if (oai_settings.max_context_unlocked) {
            ...
            const ctxProp =
                ...
                model.properties.find((p: any) => p.property_id === 'context_window');
            const contextLength = ctxProp ? Number(ctxProp.value) : max_8k;
            // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
            document
                .getElementById('openai_max_context')
                .setAttribute('max', String(contextLength || max_8k));
        }
        oai_settings.openai_max_context = Math.min(
            Number(document.getElementById('openai_max_context')?.getAttribute('max') ?? 0),
            oai_settings.openai_max_context,
        ...
        (() => {
            const el = document.getElementById('temp_openai');
            if (el) {
                el.setAttribute('max', String(workersAiMaxTemp));
                (el as any).value = oai_settings.temp_openai;
                el.dispatchEvent(new Event('input'));
            }
        })();
    ...
    if (oai_settings.chat_completion_source === chat_completion_sources.XAI) {
        if (oai_settings.max_context_unlocked) {
            ...
            // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
            document.getElementById('openai_max_context').setAttribute('max', String(max_32k));
        } else if (oai_settings.xai_model.includes('grok-4-fast')) {
            // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
            document.getElementById('openai_max_context').setAttribute('max', String(max_2mil));
    ...
    if (oai_settings.chat_completion_source === chat_completion_sources.AIMLAPI) {
        ...
        (() => {
            const el = document.getElementById('openai_max_context') as HTMLInputElement | null;
            if (el) {
                el.setAttribute('max', String(maxContext));
                el.value = String(Math.min(Number(oai_settings.openai_max_context), maxContext));
                el.dispatchEvent(new Event('input'));
            }
        })();

        (() => {
            const el = document.getElementById('temp_openai') as HTMLInputElement | null;
            if (el) {
                el.setAttribute('max', String(oai_max_temp));
                el.value = String(Number(oai_settings.temp_openai));
                el.dispatchEvent(new Event('input'));
            }
        })();
    ...
    if (oai_settings.chat_completion_source === chat_completion_sources.COHERE) {
        ...
        (() => {
            const el = document.getElementById('pres_pen_openai');
            if (el) {
                el.setAttribute('max', String(1));
                el.setAttribute('min', String(0));
                (el as any).value = oai_settings.pres_pen_openai;
                el.dispatchEvent(new Event('input'));
            }
        })();
        oai_settings.freq_pen_openai = Math.min(Math.max(0, oai_settings.freq_pen_openai), 1);
        (() => {
            const el = document.getElementById('freq_pen_openai');
            if (el) {
                el.setAttribute('max', String(1));
                el.setAttribute('min', String(0));
                (el as any).value = oai_settings.freq_pen_openai;
                el.dispatchEvent(new Event('input'));
            }
        })();
    } else {
        (() => {
            const el = document.getElementById('pres_pen_openai');
            if (el) {
                el.setAttribute('max', String(2));
                el.setAttribute('min', String(-2));
                (el as any).value = oai_settings.pres_pen_openai;
                el.dispatchEvent(new Event('input'));
            }
        })();
    ...
    // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
    document
        .getElementById('openai_max_context_counter')
        .setAttribute(
            'max',
            String(Number(document.getElementById('openai_max_context')?.getAttribute('max') ?? 0)),
        );
...
function onReverseProxyInput(this: HTMLInputElement) {
    ...
    const _rpEl2 = document.querySelector('.reverse_proxy_warning') as HTMLElement;
    if (_rpEl2) _rpEl2.style.display = oai_settings.reverse_proxy != '' ? '' : 'none';
    saveSettingsDebounced();
}
...
async function onConnectButtonClick(e) {
    ...
    if (
        ...
        if (
            !(secret_state as Record<string, unknown>)[
                SECRET_KEYS.VERTEXAI_SERVICE_ACCOUNT as string
            ]
        ) {
            notyf.error(
    ...
    const config =
        apiSourceConfig[oai_settings.chat_completion_source as keyof typeof apiSourceConfig];
    if (config) {
        const apiKey = String(
            (document.querySelector(config.selector) as HTMLInputElement | null)?.value ?? '',
        ).trim();
        if (apiKey.length) {
    ...
    for (const el of document.querySelectorAll('[data-source]')) {
        ...
        (() => {
            htmlEl.style.display =
                mode !== 'except' ? (matchesSource ? '' : 'none') : !matchesSource ? '' : 'none';
        })();
    }
...
async function onCustomizeParametersClick() {
    ...
    const customIncludeBody = template.querySelector(
        '#custom_include_body',
    ) as HTMLInputElement | null;
    ...
    const customExcludeBody = template.querySelector(
        '#custom_exclude_body',
    ) as HTMLInputElement | null;
    ...
    const customIncludeHeaders = template.querySelector(
        '#custom_include_headers',
    ) as HTMLInputElement | null;
...
function setProxyPreset(name, url, password) {
    ...
    // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
    document.getElementById('openai_reverse_proxy_name').value = name;
    oai_settings.reverse_proxy = url;
    // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
    document.getElementById('openai_reverse_proxy').value = oai_settings.reverse_proxy;
    oai_settings.proxy_password = password;
    // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
    document.getElementById('openai_proxy_password').value = oai_settings.proxy_password;
    reconnectOpenAi();
}
...
document.getElementById('save_proxy')?.addEventListener('click', async function () {
    ...
    // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
    const reverseProxy = document.getElementById('openai_reverse_proxy').value;
    // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
    const proxyPassword = document.getElementById('openai_proxy_password').value;
...
document.getElementById('delete_proxy')?.addEventListener('click', async function () {
    ...
    if (index !== -1) {
        ...
        // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
        document.getElementById('openai_reverse_proxy_name').value = selected_proxy.name;
        // @ts-expect-error TS(2532) FIXME: Object is possibly 'undefined'.
        oai_settings.reverse_proxy = selected_proxy.url;
        // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
        document.getElementById('openai_reverse_proxy').value = selected_proxy.url;
        // @ts-expect-error TS(2532) FIXME: Object is possibly 'undefined'.
        oai_settings.proxy_password = selected_proxy.password;
        // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
        document.getElementById('openai_proxy_password').value = selected_proxy.password;
...
function runProxyCallback(_: string, value: string) {
    ...
    (() => {
        const el = document.getElementById('openai_proxy_preset') as HTMLSelectElement | null;
        if (el) {
            el.value = foundName;
            el.dispatchEvent(new Event('change'));
        }
...
async function onVertexAIValidateServiceAccount() {
    const jsonContent = String(
        (document.getElementById('vertexai_service_account_json') as HTMLInputElement | null)
            ?.value ?? '',
    ).trim();
...
function onVertexAIServiceAccountJsonChange() {
    // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
    const jsonContent = String(this.value).trim();
...
function updateVertexAIServiceAccountStatus(isValid = false, message = '') {
    ...
    if (isValid && message) {
        if (infoSpan)
            infoSpan.innerHTML = `<i class="fa-solid fa-check-circle" style="color: green;"></i> ${message}`;
        if (statusDiv) statusDiv.style.display = '';
    } else if (!isValid && message) {
        if (infoSpan)
            infoSpan.innerHTML = `<i class="fa-solid fa-exclamation-triangle" style="color: orange;"></i> ${message}`;
        if (statusDiv) statusDiv.style.display = '';
    } else {
...
export function initOpenAI() {
    ...
    document.getElementById('temp_openai')?.addEventListener('input', function () {
        ...
        // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
        document.getElementById('temp_counter_openai').value = parseFloat(this.value).toFixed(2);
        saveSettingsDebounced();
    });
    ...
    document.getElementById('freq_pen_openai')?.addEventListener('input', function () {
        ...
        // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
        document.getElementById('freq_pen_counter_openai').value = parseFloat(this.value).toFixed(
            2,
        );
        saveSettingsDebounced();
    });
    ...
    document.getElementById('pres_pen_openai')?.addEventListener('input', function () {
        ...
        // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
        document.getElementById('pres_pen_counter_openai').value = parseFloat(this.value).toFixed(
            2,
        );
        saveSettingsDebounced();
    });
    ...
    document.getElementById('top_p_openai')?.addEventListener('input', function () {
        ...
        // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
        document.getElementById('top_p_counter_openai').value = parseFloat(this.value).toFixed(2);
        saveSettingsDebounced();
    });

    document.getElementById('top_k_openai')?.addEventListener('change', function () {
        // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
        oai_settings.top_k_openai = Number(this.value);
        // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
        document.getElementById('top_k_counter_openai').value = parseFloat(this.value).toFixed(0);
        saveSettingsDebounced();
    });
    ...
    document.getElementById('top_a_openai')?.addEventListener('input', function () {
        ...
        // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
        document.getElementById('top_a_counter_openai').value = Number(this.value);
        saveSettingsDebounced();
    });
    ...
    document.getElementById('min_p_openai')?.addEventListener('input', function () {
        ...
        // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
        document.getElementById('min_p_counter_openai').value = Number(this.value);
        saveSettingsDebounced();
    });
    ...
    document.getElementById('repetition_penalty_openai')?.addEventListener('input', function () {
        ...
        // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
        document.getElementById('repetition_penalty_counter_openai').value = Number(this.value);
        saveSettingsDebounced();
    });
    ...
    document.getElementById('openai_max_context')?.addEventListener('input', function () {
        ...
        // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
        document.getElementById('openai_max_context_counter').value = `${this.value}`;
        calculateOpenRouterCost();
        calculateElectronHubCost();
        calculateChutesCost();
        saveSettingsDebounced();
    ...
    document.getElementById('send_if_empty_textarea')?.addEventListener('input', function () {
        oai_settings.send_if_empty = String(
            (document.getElementById('send_if_empty_textarea') as HTMLTextAreaElement).value,
        );
        saveSettingsDebounced();
    });
    ...
    document.getElementById('newchat_prompt_textarea')?.addEventListener('input', function () {
        oai_settings.new_chat_prompt = String(
            (document.getElementById('newchat_prompt_textarea') as HTMLTextAreaElement).value,
        );
        saveSettingsDebounced();
    });
    ...
    document.getElementById('newgroupchat_prompt_textarea')?.addEventListener('input', function () {
        oai_settings.new_group_chat_prompt = String(
            (document.getElementById('newgroupchat_prompt_textarea') as HTMLTextAreaElement).value,
        );
        saveSettingsDebounced();
    });
    ...
    document.getElementById('scenario_format_textarea')?.addEventListener('input', function () {
        oai_settings.scenario_format = String(
            (document.getElementById('scenario_format_textarea') as HTMLTextAreaElement).value,
        );
        saveSettingsDebounced();
    });
    ...
    document.getElementById('personality_format_textarea')?.addEventListener('input', function () {
        oai_settings.personality_format = String(
            (document.getElementById('personality_format_textarea') as HTMLTextAreaElement).value,
        );
        saveSettingsDebounced();
    });
    ...
    document.getElementById('group_nudge_prompt_textarea')?.addEventListener('input', function () {
        oai_settings.group_nudge_prompt = String(
            (document.getElementById('group_nudge_prompt_textarea') as HTMLTextAreaElement).value,
        );
        saveSettingsDebounced();
    });
    ...
    document.getElementById('impersonation_prompt_restore')?.addEventListener('click', function () {
        ...
        ) as HTMLTextAreaElement | null)!.value = oai_settings.impersonation_prompt;
        saveSettingsDebounced();
    });
    ...
    document.getElementById('newchat_prompt_restore')?.addEventListener('click', function () {
        ...
        // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
        document.getElementById('newchat_prompt_textarea').value = oai_settings.new_chat_prompt;
        saveSettingsDebounced();
    });
    ...
    document.getElementById('newgroupchat_prompt_restore')?.addEventListener('click', function () {
        ...
        // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
        document.getElementById('newgroupchat_prompt_textarea').value =
            oai_settings.new_group_chat_prompt;
        saveSettingsDebounced();
    });

    document
        .getElementById('newexamplechat_prompt_restore')
        ?.addEventListener('click', function () {
            oai_settings.new_example_chat_prompt = default_new_example_chat_prompt;
            // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
            document.getElementById('newexamplechat_prompt_textarea').value =
                oai_settings.new_example_chat_prompt;
            saveSettingsDebounced();
        });

    document
        .getElementById('continue_nudge_prompt_restore')
        ?.addEventListener('click', function () {
            oai_settings.continue_nudge_prompt = default_continue_nudge_prompt;
            // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
            document.getElementById('continue_nudge_prompt_textarea').value =
    ...
    document.getElementById('wi_format_restore')?.addEventListener('click', function () {
        ...
        // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
        document.getElementById('wi_format_textarea').value = oai_settings.wi_format;
        saveSettingsDebounced();
    });
    ...
    document.getElementById('scenario_format_restore')?.addEventListener('click', function () {
        ...
        // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
        document.getElementById('scenario_format_textarea').value = oai_settings.scenario_format;
        saveSettingsDebounced();
    });
    ...
    document.getElementById('personality_format_restore')?.addEventListener('click', function () {
        ...
        // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
        document.getElementById('personality_format_textarea').value =
    ...
    document.getElementById('group_nudge_prompt_restore')?.addEventListener('click', function () {
        ...
        // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
        document.getElementById('group_nudge_prompt_textarea').value =
    ...
    document.getElementById('chat_completion_source')?.addEventListener('change', function () {
        ...
        model_list = [];
        // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
        oai_settings.chat_completion_source = String(this.selectedOptions[0].value);
    ...
    document
        .getElementById('claude_assistant_impersonation')
        ?.addEventListener('input', function () {
            // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
    ...
    document.getElementById('tool_call_recurse_limit')?.addEventListener('input', function () {
        ...
        // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
        document.getElementById('tool_call_recurse_limit_counter').value =
    ...
    document.getElementById('tool_reasoning_mode')?.addEventListener('input', function () {
        oai_settings.tool_reasoning_mode = getToolReasoningMode({
            ...oai_settings,
            // @ts-expect-error TS(2592) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
            tool_reasoning_mode: String(this.value),
        });
        saveSettingsDebounced();
    });
    ...
    document
        .getElementById('custom_prompt_post_processing')
    ...
    document
        .getElementById('continue_postfix_double_newline')
        ?.addEventListener('input', function () {
    ...
    if (!CSS.supports('field-sizing', 'content')) {
        document.addEventListener('input', function (this: HTMLElement, e: Event) {
            const target = (e.target as HTMLElement)?.closest('#openai_settings .autoSetHeight');
            if (!target) return;
    ...
    if (!isMobile()) {
        ...
        new TomSelect(
            document.getElementById('completion_prompt_manager_popup_entry_form_injection_trigger'),
            {
                maxItems: null,
    ...
    document
        ...
        ?.addEventListener('change', function (this: HTMLSelectElement) {
            ...
            (oai_settings as Record<string, unknown>).openrouter_quantizations =
    ...
    document.getElementById('cc_group_models')?.addEventListener('input', async () => {
        oai_settings.group_models =
            (document.getElementById('cc_group_models') as HTMLInputElement | null)?.checked ??
            false;
        reconnectOpenAi();
        saveSettingsDebounced();
    });
    ...
    document.getElementById('cc_sort_models')?.addEventListener('input', async () => {
        oai_settings.sort_models =
            (
                document.getElementById('cc_sort_models') as HTMLInputElement | null
            )?.value?.toString() ?? '';
        reconnectOpenAi();
        saveSettingsDebounced();
    });
    ...
    document
        .getElementById('vertexai_auth_mode')
        ?.addEventListener('change', onVertexAIAuthModeChange);
    ...
    document
        .getElementById('vertexai_service_account_json')
        ?.addEventListener('input', onVertexAIServiceAccountJsonChange);
    document
        .getElementById('vertexai_validate_service_account')
        ?.addEventListener('click', onVertexAIValidateServiceAccount);
    document
        .getElementById('vertexai_clear_service_account')
        ?.addEventListener('click', onVertexAIClearServiceAccount);
    ...
    document
        .getElementById('settings_preset_openai')
        ?.addEventListener('change', onSettingsPresetChange);
    ...
    document
        .getElementById('openai_logit_bias_preset')
        ?.addEventListener('change', onLogitBiasPresetChange);
    document
        .getElementById('openai_logit_bias_new_preset')
        ?.addEventListener('click', createNewLogitBiasPreset);
    document
        .getElementById('openai_logit_bias_new_entry')
        ?.addEventListener('click', createNewLogitBiasEntry);
    document
        .getElementById('openai_logit_bias_import_file')
        ?.addEventListener('input', onLogitBiasPresetImportFileChange);
    document
        .getElementById('openai_preset_import_file')
        ?.addEventListener('input', onPresetImportFileChange);
    document.getElementById('export_oai_preset')?.addEventListener('click', onExportPresetClick);
    document
        .getElementById('openai_logit_bias_import_preset')
        ?.addEventListener('click', onLogitBiasPresetImportClick);
    document
        .getElementById('openai_logit_bias_export_preset')
        ?.addEventListener('click', onLogitBiasPresetExportClick);
    document
        .getElementById('openai_logit_bias_delete_preset')
        ?.addEventListener('click', onLogitBiasPresetDeleteClick);
    document.getElementById('import_oai_preset')?.addEventListener('click', onImportPresetClick);
    document
        .getElementById('openai_proxy_password_show')
        ?.addEventListener('click', onProxyPasswordShowClick);
    document
        .getElementById('customize_additional_parameters')
        ?.addEventListener('click', onCustomizeParametersClick);
