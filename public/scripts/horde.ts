 ...
 * @throws {Error}
 */
export async function generateHorde(prompt: string, params: Record<string, unknown>, signal: AbortSignal, reportProgress?: (progress: number) => void) {
    validateHordeModel();
...
export async function getHordeModels(force: boolean) {
    ...
    const sortByWhitelisted = (a: HordeModel, b: HordeModel) => Number(b.is_whitelisted) - Number(a.is_whitelisted);
    const sortByPopular = (a: HordeModel, b: HordeModel) => Number(b.tags?.includes('popular')) - Number(a.tags?.includes('popular'));

    const hordeModelSelect = document.getElementById('horde_model');
    if (hordeModelSelect) hordeModelSelect.innerHTML = '';
    models = ((await getModels(force)) as HordeModel[]).sort((a, b) => {
...
export function loadHordeSettings(settings: Record<string, unknown>) {
    ...
    const el1 = document.getElementById('horde_auto_adjust_response_length') as HTMLInputElement | null; if (el1) el1.checked = horde_settings.auto_adjust_response_length;
    const el2 = document.getElementById('horde_auto_adjust_context_length') as HTMLInputElement | null; if (el2) el2.checked = horde_settings.auto_adjust_context_length;
    const el3 = document.getElementById('horde_trusted_workers_only') as HTMLInputElement | null; if (el3) el3.checked = horde_settings.trusted_workers_only;
}
