 ...
 * @param force
 */
async function getWorkers(force: boolean) {
 ...
 */
async function getModels(force: boolean) {
 ...
 *
 */
function validateHordeModel(): HordeModel[] {
 ...
 * @param max_length
 */
export async function adjustHordeGenerationParams(max_context_length: number, max_length: number) {
    ...
    document.getElementById('adjustedHordeParams')!.textContent =
 ...
 */
export async function getHordeModels(force: boolean) {
    const sortByPerformance = (a: HordeModel, b: HordeModel) => b.performance - a.performance;
    const sortByWhitelisted = (a: HordeModel, b: HordeModel) =>
        Number(b.is_whitelisted) - Number(a.is_whitelisted);
    const sortByPopular = (a: HordeModel, b: HordeModel) =>
        Number(b.tags?.includes('popular')) - Number(a.tags?.includes('popular'));

    const hordeModelSelect = document.getElementById('horde_model');
    if (hordeModelSelect) hordeModelSelect.innerHTML = '';
    models = ((await getModels(force)) as HordeModel[]).toSorted((a, b) => {
    ...
    if (
        ...
        horde_settings.models = [] as string[];
    }
 ...
 * @param settings
 */
export function loadHordeSettings(settings: Record<string, unknown>) {
    if (settings.horde_settings) {
        Object.assign(horde_settings, settings.horde_settings);
    }

    const el1 = document.getElementById(
        'horde_auto_adjust_response_length',
    ) as HTMLInputElement | null;
    if (el1) el1.checked = horde_settings.auto_adjust_response_length;
    const el2 = document.getElementById(
        'horde_auto_adjust_context_length',
    ) as HTMLInputElement | null;
    if (el2) el2.checked = horde_settings.auto_adjust_context_length;
    const el3 = document.getElementById('horde_trusted_workers_only') as HTMLInputElement | null;
    if (el3) el3.checked = horde_settings.trusted_workers_only;
}
 ...
 * @param model
 */
function hordeModelTextString(model: HordeModel) {
 ...
 * @param model
 */
function hordeModelQueueStateString(model: HordeModel) {
...
function getHordeModelTemplate(option: Record<string, unknown>) {
    ...
    const tagSpans =
        (tags.length > 0 &&
            `${tags.map((tag: string) => `<span class="tag tag_name">${tag}</span>`).join('')}</span>`) ||
        '';

    const modelDetailsLink = url
...
export function initHorde() {
    const hordeModelSelect = document.getElementById('horde_model');
    if (hordeModelSelect) {
        hordeModelSelect.addEventListener(
            'mousedown',
        ...
        hordeModelSelect.addEventListener('change', async function (this: HTMLSelectElement) {
            ...
            (horde_settings.models as string[]) = modelValue;
    ...
    document
        .getElementById('horde_auto_adjust_response_length')
    ...
    document
        .getElementById('horde_auto_adjust_context_length')
        ?.addEventListener('input', function (this: HTMLInputElement) {
    ...
    document
        .getElementById('horde_trusted_workers_only')
        ?.addEventListener('input', function (this: HTMLInputElement) {
    ...
    document.getElementById('horde_api_key_button')?.addEventListener('click', async function () {
        const key = String(
            (document.getElementById('horde_api_key') as HTMLInputElement | null)?.value ?? '',
        ).trim();
        if (!key) {
    ...
    if (!isMobile()) {
        ...
        new (TomSelect as any)(document.getElementById('horde_model'), {
            maxItems: null,
            placeholder: t`Select Horde models`,
            allowEmptyOption: true,
            plugins: ['remove_button'],
            render: {
                option: function (data: unknown, escape: unknown) {
                    return getHordeModelTemplate(data as Record<string, unknown>);
                },
            },
        });
    }
