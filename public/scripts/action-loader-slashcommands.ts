    ...
    function createClosureHandler(closure, { argName = 'onStop', throwInvalid = true } = {}) {
        if (!(closure instanceof SlashCommandClosure)) {
            if (closure && throwInvalid) {
                ...
                throw new Error(
                    t`Invalid argument for ${argName} provided. This is not a closure.`,
                );
            }
            return null;
        }
        ...
        `,
            namedArgumentList: [
                ...
                SlashCommandNamedArgument.fromProps({
                    name: 'slug',
                    description:
                        'Unique slug for the loader (to identify it easily via code or CSS)',
                    typeList: [ARGUMENT_TYPE.STRING],
                ...
                SlashCommandNamedArgument.fromProps({
                    name: 'stopTooltip',
                    description:
                        'Tooltip text for the stop button (only used when toast=stoppable)',
                    typeList: [ARGUMENT_TYPE.STRING],
                    defaultValue: 'Stop',
                }),
                SlashCommandNamedArgument.fromProps({
