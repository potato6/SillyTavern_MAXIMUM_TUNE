    ...
    test.describe('{{pipe}} Macro', () => {
        test('should support {{pipe}} macro', async ({ page }) => {
            const output = await page.evaluate(async () => {
                ...
                return (
                    await executeSlashCommandsWithOptions('/pass Hello World || /pass {{pipe}}')
                ).pipe;
            });
    ...
    test.describe('{{arg}} Macro', () => {
        test('should support {{arg}} macro with plain value', async ({ page }) => {
            const output = await page.evaluate(async () => {
                ...
                return (
                    await executeSlashCommandsWithOptions(
                        '/qr-arg hello world || /pass {{arg::hello}}',
                    )
                ).pipe;
            });
    ...
    test.describe('Custom Scope Macros', () => {
        test('should support {{timesIndex}} in /times command', async ({ page }) => {
            const output = await page.evaluate(async () => {
                ...
                return (
                    await executeSlashCommandsWithOptions('/times 1 {: /pass {{timesIndex}} :}')
                ).pipe;
            });
        ...
        test('should support custom SlashCommandScope macros', async ({ page }) => {
            const output = await page.evaluate(async () => {
                ...
                const { SlashCommandScope } =
                    await import('./scripts/slash-commands/SlashCommandScope.js');
                const { power_user } = await import('./scripts/power-user.js');
                ...
                return (
                    await executeSlashCommandsWithOptions('/pass {{uno::dos::tres}}', {
                        scope: customScope,
                    })
                ).pipe;
            });
