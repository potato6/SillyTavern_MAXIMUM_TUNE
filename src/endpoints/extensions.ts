...
router.post('/install', async (request, response) => {
    try {
        ...
        if (!isValidUrl(url)) {
            return response
                .status(400)
                .send('Bad Request: A valid URL is required in the request body.');
        }
        ...
        if (!['http:', 'https:'].includes(parsedUrl.protocol)) {
            return response
                .status(400)
                .send(
                    'Bad Request: Only HTTP and HTTPS protocols are supported for the Extension URL.',
                );
        }
        ...
        if (branch) {
            // @ts-expect-error TS(2339) FIXME: Property 'branch' does not exist on type '{ depth:... Remove this comment to see the full error message
    ...
    try {
        ...
        if (!extensionNameSanitized || !branch) {
            return response
                .status(400)
                .send(
                    'Bad Request: A valid extensionName and branch are required in the request body.',
                );
        }
